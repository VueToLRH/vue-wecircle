...
export default {
  ...
  computed: {
    ...
    imgOneStyle () {
      return item => {
        ...
        return {
          height: this.pxtovw(height), // 转换成vw单位
  ...
  methods: {
    ...
    // 点赞
    async addLike () {
      let resp = await service.post('likecomment/addlike', { postId: this.data.id })
      if (resp.code === 0) {
    ...
    // 取消点赞
    async removeLike () {
      let resp = await service.post('likecomment/removelike', { postId: this.data.id })
      if (resp.code === 0) {
